...
let rec process list =
  ...
  match list with
  | [] -> []
  | hd :: tl -> (rank hd) :: (process tl)
;;

let rec find_pass list = match list with
  | [] -> (-1, -1, -1)
  | hd :: he :: tl when he <> (hd + 1) -> (hd, he, (hd + he)/2)
  | hd :: tl -> find_pass tl
;;

find_pass (List.sort compare (process (readfile "./input.txt")));;
